...
/// <summary>
/// 鎖住 GasMeterLeafAggregator 純邏輯：boundary 相減、MaxVolume 溢位歸零、UnitScale L→m³ 換算、
/// 邊界缺值三段語意（兩缺不寫 / 缺一邊 Q=0 / 兩有 Q=1）。
/// 對錯 = 用氣報表與氣費的每一度氣都算錯。與水表 WaterMeterLeafAggregator 語意對稱。
/// </summary>
public class GasMeterLeafAggregatorTests
{
    private static readonly DateTime Hour = new(2026, 8, 5, 10, 0, 0);

    // ── CalcDeltaWithRollover：boundary 相減 + 溢位 ──────────────────

    [Theory]
    [InlineData(100.0, 130.5, 30.5)]   // 正常累計
    [InlineData(0.0, 0.0, 0.0)]        // 無用氣
    [InlineData(999.9, 999.9, 0.0)]    // 讀數不動
    public void 正常累計_結尾減開頭(double start, double end, double expected)
    {
        var (dDelta, isRolledOver) = GasMeterLeafAggregator.CalcDeltaWithRollover(start, end, 10000);
    ...
    public void 溢位歸零_有MaxVolume_套環繞公式()
    {
        ...
        var (dDelta, isRolledOver) = GasMeterLeafAggregator.CalcDeltaWithRollover(9990, 15, 10000);
    ...
    {
        var (dDelta, isRolledOver) = GasMeterLeafAggregator.CalcDeltaWithRollover(500, 20, maxVolume);
    ...
    {
        var model = GasMeterLeafAggregator.ComputeFromBoundaries("G1-S1", Hour, null, null, 10000, 1.0);
        Assert.Null(model);
    }
    ...
    public void 只缺一邊_掉線transition_Q0Delta0(bool missingStart)
    {
        ...
        var model = GasMeterLeafAggregator.ComputeFromBoundaries("G1-S1", Hour, fStart, fEnd, 10000, 1.0);

        Assert.NotNull(model);
        Assert.Equal(0, model!.nQuality);
        Assert.Equal(0, model.dDeltaM3, 10);
        Assert.False(model.isRolledOver);
        Assert.Equal("G1-S1", model.szSID);
    ...
    {
        var model = GasMeterLeafAggregator.ComputeFromBoundaries("G1-S1", Hour, 120.0, 123.5, 10000, 1.0);
    ...
    public void UnitScale_公升點位_換算成立方米()
    {
        ...
        var model = GasMeterLeafAggregator.ComputeFromBoundaries("G1-S2", Hour, 20000.0, 21500.0, null, 0.001);
    ...
    [Fact]
    public void UnitScale_度數點位_係數1直接視為立方米()
    {
        // 天然氣帳單 1 度 = 1 m³ → UnitScale=1.0，讀數差即 m³
        var model = GasMeterLeafAggregator.ComputeFromBoundaries("G1-S3", Hour, 1200.0, 1245.0, null, 1.0);

        Assert.NotNull(model);
        Assert.Equal(45.0, model!.dDeltaM3, 10);
    }
    ...
    public void UnitScale_溢位先以原始單位計算再換算()
    {
        ...
        var model = GasMeterLeafAggregator.ComputeFromBoundaries("G1-S2", Hour, 99000.0, 500.0, 100000, 0.001);
