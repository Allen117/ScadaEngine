...
/// <summary>
/// 用水報表 — 葉子層 hourly 預聚合資料模型（對應 WaterMeterLeafHourly 表）。
/// 一列 = 一個葉子 SID 在某個小時的用水量增量（m³，已套 UnitScale 換算與 MaxVolume 溢位規則，未套 sign）。
/// Quality=0 表示「掉線事件 transition」（只缺一邊邊界值），DeltaM3=0。
/// 兩邊都缺 → 不寫該列（sparse storage）。
/// </summary>
public class WaterMeterLeafHourlyModel
{
    ...
    /// <summary>該小時用水量增量（m³，已套 UnitScale 與溢位規則，未套 sign）。Quality=0 時恆為 0。</summary>
