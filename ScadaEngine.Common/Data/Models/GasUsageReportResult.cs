...
/// <summary>
/// 用氣報表查詢結果 — 對應某氣表迴路在某粒度下的 N 個 bucket。
/// 純資料結構，由 GasUsageReportService 產出，View / Excel 匯出使用。
/// </summary>
public class GasUsageReportResult
{
    ...
    /// <summary>各 bucket 的時間標籤與用氣量（m³）</summary>
    public List<GasUsageReportBucket> buckets { get; set; } = new();

    /// <summary>區間總用氣量（m³，所有 bucket 加總）</summary>
    ...
    /// </summary>
    public List<GasUsageReportChildSeries> children { get; set; } = new();
}

/// <summary>單一 bucket 的時間 + 用氣量</summary>
public class GasUsageReportBucket
{
...
/// </summary>
public class GasUsageReportChildSeries
{
    ...
    /// <summary>各 bucket 用氣量（m³），與父 buckets 同 index 對齊</summary>
