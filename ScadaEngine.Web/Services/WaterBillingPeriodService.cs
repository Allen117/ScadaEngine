...
/// <summary>
/// 水費月結週期（期別）— 用水報表 / 水費報表 / EMS 水表卡的唯一期界來源。
/// 與電費期別（<see cref="BillingPeriodService"/> / <c>BillingPeriods</c>）**完全獨立**：
/// 台水與台電抄表日不同，兩套各自設定，互不影響。
///
/// 期別 M 的解析規則（與電費版相同）：
///   1. WaterBillingPeriods 有 row → 直接採用（使用者自訂）
...
///
/// ⚠️ 快取欄位 <c>_cachedRows</c> 必須與電費版**各自獨立**（見下方註解）。
/// </summary>
public class WaterBillingPeriodService
{
    private readonly ILogger<WaterBillingPeriodService> _logger;
    private readonly DatabaseConfigService _configService;
    private string _szConnectionString = string.Empty;

    // 全自訂 row 快取 — 表小、讀多寫少；static 跨 Scoped 實例共用，寫入時失效。
    // ⚠️ 這個欄位刻意與 BillingPeriodService._cachedRows 分開宣告（不抽共用基底類別）：
    //    static 欄位會被基底類別的所有子類共享，一旦共用會造成「改電費期別污染水費期別」，
    //    症狀是偶爾抓到對方的期別，極難除錯。
    private static volatile Dictionary<(int nYear, int nMonth), BillingPeriodModel>? _cachedRows;

    public WaterBillingPeriodService(ILogger<WaterBillingPeriodService> logger, DatabaseConfigService configService)
    {
        _logger = logger;
    ...
    /// <summary>
    /// 取得期別區間 [fromYM, toYM]（含頭尾）每期一對 [起, 訖) 邊界 — 用水報表月粒度 bucket 來源。
    ...
    /// <summary>
    /// 今天所屬期別 — 用水報表日粒度預設起訖、EMS 水費卡「本期」用。
    ...
    public async Task SaveAsync(int nYear, int nMonth, DateTime dtStartDate, DateTime dtEndDate)
    {
        ...
        const string szSql = @"
            MERGE WaterBillingPeriods AS t
        ...
        _cachedRows = null;
        _logger.LogInformation("水費月結週期已更新 {Year}-{Month:00}: {Start:yyyy-MM-dd} ~ {End:yyyy-MM-dd}",
    ...
    {
        const string szSql = "DELETE FROM WaterBillingPeriods WHERE PeriodYear = @nYear AND PeriodMonth = @nMonth";
        using var conn = await GetConnectionAsync();
    ...
    /// 格式與電費版一致（<see cref="BillingPeriodService.BuildLabel"/>）。
    /// </summary>
