...
/// <summary>
/// 用氣報表 Excel 匯出 — 使用 ClosedXML（無 Office 依賴）。
/// 獨立檔案，便於日後抽換套件。
/// 透過 IStringLocalizer 取得當前 culture 的標題、表頭、警告等字串。
/// </summary>
public class GasUsageReportExcelExporter
{
    private readonly IStringLocalizer<GasUsageReportExcelExporter> _l;

    public GasUsageReportExcelExporter(IStringLocalizer<GasUsageReportExcelExporter> localizer)
    {
        _l = localizer;
    }

    /// <summary>產出 .xlsx 二進位內容</summary>
    public byte[] Export(GasUsageReportResult result, string szOperator)
    {
