...
/// <summary>
/// 用水報表 Excel 匯出 — 使用 ClosedXML（無 Office 依賴）。
/// 獨立檔案，便於日後抽換套件。
/// 透過 IStringLocalizer 取得當前 culture 的標題、表頭、警告等字串。
/// </summary>
public class WaterUsageReportExcelExporter
{
    private readonly IStringLocalizer<WaterUsageReportExcelExporter> _l;

    public WaterUsageReportExcelExporter(IStringLocalizer<WaterUsageReportExcelExporter> localizer)
    {
        _l = localizer;
    }

    /// <summary>產出 .xlsx 二進位內容</summary>
    public byte[] Export(WaterUsageReportResult result, string szOperator)
    {
        ...
        ws.Cell(8, 1).Value = _l["excel.label.total_m3"].Value;
        ws.Cell(8, 2).Value = result.dTotalM3;
        ...
        if (!bHasChildren)
        {
            // 實體葉子或無子迴路時，欄名維持「用水量 (m³)」
            ws.Cell(nDataStartRow, 2).Value = _l["excel.col.m3"].Value;
        }
        else
        {
            // 父欄改用迴路名稱明確標示，避免與子欄混淆
            ws.Cell(nDataStartRow, 2).Value = _l["excel.col.with_circuit_m3", result.szCircuitName].Value;
            for (var i = 0; i < result.children.Count; i++)
            {
                ws.Cell(nDataStartRow, 3 + i).Value = _l["excel.col.with_circuit_m3", result.children[i].szName].Value;
            }
        }
        ...
        for (var i = 0; i < result.buckets.Count; i++)
        {
            ...
            ws.Cell(row, 2).Value = result.buckets[i].dM3;
            ws.Cell(row, 2).Style.NumberFormat.Format = "#,##0.000";
            if (bHasChildren)
            {
                for (var c = 0; c < result.children.Count; c++)
                {
                    var dVal = i < result.children[c].dM3PerBucket.Count ? result.children[c].dM3PerBucket[i] : 0;
        ...
        ws.Cell(sumRow, 2).Value = result.dTotalM3;
        ws.Cell(sumRow, 2).Style.NumberFormat.Format = "#,##0.000";
        if (bHasChildren)
        {
            for (var c = 0; c < result.children.Count; c++)
            {
                ws.Cell(sumRow, 3 + c).Value = result.children[c].dTotalM3;
