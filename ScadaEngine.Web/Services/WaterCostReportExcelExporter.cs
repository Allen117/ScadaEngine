...
/// <summary>
/// 水費報表 Excel 匯出 — 使用 ClosedXML，格式比照 ElectricityCostReportExcelExporter。
/// 表格為 期別 / 用水量 (m³) / 套用方案 / 水費（元）；資料不完整期別於期別欄加「*」註記。
/// 透過 IStringLocalizer 取得當前 culture 字串。
/// </summary>
public class WaterCostReportExcelExporter
{
    private const string CostFormat = "#,##0.0";
    private const string M3Format = "#,##0.00";

    private readonly IStringLocalizer<WaterCostReportExcelExporter> _l;

    public WaterCostReportExcelExporter(IStringLocalizer<WaterCostReportExcelExporter> localizer)
    {
        _l = localizer;
    }
    ...
    public byte[] Export(string szCircuitName, string szFromYm, string szToYm,
        List<WaterCostPeriodRow> rows, string szOperator)
    {
