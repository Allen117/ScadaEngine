...
using ScadaEngine.Web.Features.WaterBillingPeriodSetting.Models;
using ScadaEngine.Web.Services;

namespace ScadaEngine.Web.Features.WaterBillingPeriodSetting.Controllers;

/// <summary>
/// 水費月結週期設定 — 每期（YYYY-MM）自訂起訖日期，用水/水費月粒度報表共用。
/// 與電費月結週期（<c>/BillingPeriodSetting</c>）各自獨立設定，互不影響。
/// api/current 與 api/range 為唯讀查詢，供用水報表等頁面顯示期別提示 / 帶入本期預設。
/// </summary>
[Authorize]
[Route("[controller]")]
public class WaterBillingPeriodSettingController : Controller
{
    private readonly WaterBillingPeriodService _service;
    private readonly ILogger<WaterBillingPeriodSettingController> _logger;

    public WaterBillingPeriodSettingController(WaterBillingPeriodService service, ILogger<WaterBillingPeriodSettingController> logger)
    {
        _service = service;
        _logger = logger;
    }

    [HttpGet("/WaterBillingPeriodSetting")]
    public IActionResult Index()
    {
        return View(new WaterBillingPeriodSettingViewModel());
    }
    ...
    [HttpPost("api/save")]
    public async Task<IActionResult> Save([FromBody] WaterBillingPeriodSaveRequest dto)
    {
        try
        {
        ...
        {
            _logger.LogError(ex, "水費月結週期儲存失敗 {Year}-{Month}", dto.year, dto.month);
    ...
    [HttpPost("api/reset")]
    public async Task<IActionResult> Reset([FromBody] WaterBillingPeriodResetRequest dto)
    {
        try
        {
        ...
        {
            _logger.LogError(ex, "水費月結週期還原失敗 {Year}-{Month}", dto.year, dto.month);
            return StatusCode(500, new { message = "還原失敗" });
        }
    }

    /// <summary>今天所屬期別 — 用水報表日粒度預設起訖用</summary>
    [HttpGet("api/current")]
    ...
    private static WaterBillingPeriodItemDto ToDto(BillingPeriodRange p, int nGapDays) => new()
    {
        year = p.nYear,
        month = p.nMonth,
