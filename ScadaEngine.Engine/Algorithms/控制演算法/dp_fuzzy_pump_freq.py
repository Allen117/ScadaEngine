# -*- coding: utf-8 -*-
# @algorithm: 壓差Fuzzy水泵頻率
# @variadic: true
# @inputs_fixed: p_sup:供水壓, p_ret:回水壓, dp_target:壓差目標, e_scale:誤差範圍, de_scale:變化範圍, step_max:單步最大Hz, deadband:不動作帶
# @inputs_repeat: run:運轉狀態, freq_fb:目前頻率
# @inputs_auto_repeat: freq_min:freq:Min, freq_max:freq:Max
# @inputs_default: e_scale=30, de_scale=10, step_max=2, deadband=5
# @outputs_repeat: freq:頻率輸出
# @description: 供回水壓差對目標做 5x5 Fuzzy 增量控制，N 台泵共用壓力與目標、各自輸出頻率；只有壓差計時將壓差接 p_sup、p_ret 接常數 0；Min/Max 由框架依下游點位自動注入
...
E_SCALE_FALLBACK = 30.0     # e_scale <= 0 時的防呆回退（與壓力點位同單位，預設假定 kPa）
DE_SCALE_FALLBACK = 10.0    # de_scale <= 0 時的防呆回退
STEP_MAX_FALLBACK = 2.0     # step_max <= 0 時的防呆回退（Hz）

# 5×5 規則矩陣（反對角經典型）：列 = e 隸屬 (NB..PB)、欄 = de 隸屬 (NB..PB)，
# 輸出單例 ∈ [-1, 1]。e = 目標 − 實測壓差，e 為正（壓差不足）→ Δf 為正（加速）。
# 矩陣形狀屬工程師調表領域，改此表重啟服務即生效（沿用溫差版決策 4）。
_RULES = (
    #  de:  NB     NS     ZO     PS     PB
    (-1.0, -1.0, -0.75, -0.5,   0.0),   # e = NB（壓差遠高於目標 → 大減速）
    (-1.0, -0.75, -0.5,   0.0,  0.5),   # e = NS
    (-0.75, -0.5,  0.0,   0.5,  0.75),  # e = ZO
    (-0.5,   0.0,  0.5,   0.75, 1.0),   # e = PS
    ( 0.0,   0.5,  0.75,  1.0,  1.0),   # e = PB（壓差遠低於目標 → 大加速）
)

# ── 跨呼叫狀態（時間視窗輪次快取，沿用溫差版決策 3）───────────────────
...
def _update_round(e):
    ...
    同輪第 2..N 次呼叫（毫秒內）直接沿用，避免各泵 de 不一致（沿用溫差版決策 3）。"""
...
def evaluate_one(p_sup, p_ret, dp_target, e_scale, de_scale, step_max, deadband,
                 run, freq_fb, freq_min, freq_max):
    """
    壓差 Fuzzy 水泵頻率（增量式 velocity form）：
    e = 壓差目標 − (供水壓 − 回水壓)、de = e − e_prev（跨輪），
    5×5 矩陣查增量 Δf = u × step_max，輸出 freq = clamp(freq_fb + Δf, freq_min, freq_max)。

    與溫差版方向相反：水泵加速使壓差「上升」，故 e 定義為目標 − 實測（壓差不足為正 → 加速），
    即可沿用同一張規則矩陣。壓力單位不限（kPa / bar / mH2O），e_scale、de_scale、deadband
    與壓力點位同單位即可；預設值假定 kPa。
    ...
    Status:
        ...
        OK                   — 含 deadband 內不動作與停機泵維持原值（沿用溫差版決策 5）
    """
    ...
    e = dp_target - (p_sup - p_ret)
    _update_round(e)

    # 停機泵：輸出維持原值（不推頻率、開機瞬間無擾動接手），status OK（沿用溫差版決策 5）
    if run == 0:
