...
/// <summary>
/// 水表葉子層 hourly 聚合 — 純邏輯類（累積式水表 m³/L，boundary 相減 + MaxVolume 溢位 + UnitScale 換算）。
/// 給 WaterMeterLeafAggregationService（hourly Timer）使用。
/// 輸入單一 SID + 單一小時，回傳 WaterMeterLeafHourlyModel 或 null（兩邊邊界都缺 → 不寫，sparse storage）。
/// 演算法與 EnergyLeafAggregator 對稱（kWh → m³、MaxKwh → MaxVolume，另加 UnitScale）。
/// 與 Web 端 WaterUsageReportService 的 delta 語意必須一致。
/// </summary>
public class WaterMeterLeafAggregator
{
    private readonly ILogger<WaterMeterLeafAggregator> _logger;
    private readonly DatabaseConfigService _configService;
    private string _szConnectionString = string.Empty;

    public WaterMeterLeafAggregator(
        ILogger<WaterMeterLeafAggregator> logger,
    ...
    /// <summary>
    /// 計算 sid 在 [hourStart, hourStart+1hr) 的用水量增量（m³）。
    /// 三段語意：
    ///   兩邊有 → Q=1 算 delta（原始單位相減後 × UnitScale）
    ///   只缺一邊 → Q=0, Delta=0（掉線 transition）
    ///   兩邊都缺 → 回傳 null（sparse storage，不寫該列）
    /// </summary>
    public async Task<WaterMeterLeafHourlyModel?> ComputeAsync(
        ...
        int nMaxStalenessHours, string szLeafName = "")
    {
        ...
        {
            _logger.LogWarning(
                "水表葉子層聚合 {SID} ({Name}) 偵測到累積值倒退（{Start} → {End}）但未設定 MaxVolume，該段 delta 視為 0",
    ...
    /// </summary>
    public static WaterMeterLeafHourlyModel? ComputeFromBoundaries(
        string szSid, DateTime dtHourStart, double? fStart, double? fEnd, double? dMaxVolume, double dUnitScale)
    {
        ...
        if (fStart == null || fEnd == null)
        {
            return new WaterMeterLeafHourlyModel
            {
                szSID = szSid,
    ...
    /// <summary>
    /// 水量溢位/重置 delta（以點位原始單位計）— 與電表 CalcDeltaWithRollover 語意對稱。
    /// V_end &gt;= V_start: 正常累計
    /// V_end &lt; V_start &amp;&amp; MaxVolume 有設: (Max - Vs) + Ve
    /// V_end &lt; V_start &amp;&amp; MaxVolume 無設: 視為水表重置/異常，回 0
    /// </summary>
    ...
    /// 與 EnergyLeafAggregator.GetBoundaryValuesAsync 行為一致。
    /// </summary>
