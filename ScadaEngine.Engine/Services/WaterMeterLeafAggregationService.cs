...
/// <summary>
/// 水表葉子層 hourly 預聚合背景服務。
/// 啟動時做 catch-up（過去 N 小時 × 所有葉子，缺漏才補算），主迴圈每 30 秒檢查是否到 XX:02:00，
/// 到達就針對「剛結束的上個小時」聚合一次。
///
/// Engine 完全不參與階層加總 — sign 權重、虛擬節點仍由 Web on-demand 計算，
/// 確保「迴路結構改變即時生效不需回填」。與 EnergyLeafAggregationService 對稱。
/// </summary>
public class WaterMeterLeafAggregationService : BackgroundService
{
    private readonly ILogger<WaterMeterLeafAggregationService> _logger;
    private readonly WaterMeterLeafAggregator _aggregator;
    private readonly WaterMeterLeafHourlyRepository _repository;
    private readonly IConfiguration _configuration;

    /// <summary>觸發時點（每小時的第 2 分鐘）— 給 DB 來源 polling 留 buffer</summary>
    private const int TRIGGER_MINUTE = 2;

    public WaterMeterLeafAggregationService(
        ILogger<WaterMeterLeafAggregationService> logger,
        WaterMeterLeafAggregator aggregator,
        WaterMeterLeafHourlyRepository repository,
    ...
    {
        var nMaxStalenessHours = _configuration.GetValue<int?>("WaterMeterAggregation:MaxStalenessHours") ?? 2;
        var nCatchUpHours = _configuration.GetValue<int?>("WaterMeterAggregation:CatchUpHours") ?? 24;

        _logger.LogInformation(
            "水表葉子層 hourly 預聚合服務啟動，MaxStalenessHours={Staleness}, CatchUpHours={CatchUp}, TriggerMinute=XX:{Min:D2}",
        ...
        {
            _logger.LogError(ex, "水表葉子層聚合啟動 catch-up 失敗");
        }

        // 主迴圈：每 30 秒檢查是否到 XX:02
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                _logger.LogError(ex, "水表葉子層聚合主迴圈發生錯誤");
                try { await Task.Delay(TimeSpan.FromSeconds(10), stoppingToken); }
                catch (OperationCanceledException) { break; }
            }
        }

        _logger.LogInformation("水表葉子層 hourly 預聚合服務已停止");
    }

    /// <summary>啟動時 catch-up — 過去 N 小時每葉子若 WaterMeterLeafHourly 缺對應列就補算</summary>
    private async Task CatchUpAsync(int nCatchUpHours, int nMaxStalenessHours, CancellationToken stoppingToken)
    {
        ...
        if (leaves.Count == 0)
        {
            _logger.LogInformation("水表葉子層聚合 catch-up: 無葉子節點可聚合");
            return;
        }
        ...
        _logger.LogInformation(
            "水表葉子層聚合 catch-up 開始：葉子 {LeafCount} 個，範圍 {From:yyyy-MM-dd HH:mm} ~ {To:yyyy-MM-dd HH:mm}",
        ...
        _logger.LogInformation(
            "水表葉子層聚合 catch-up 完成：寫入 {Filled} 列、已存在跳過 {Skipped} 列、sparse 跳過 {SparseSkipped} 列",
    ...
    private async Task AggregateHourAsync(DateTime dtHourStart, int nMaxStalenessHours, CancellationToken stoppingToken)
    {
        ...
        if (leaves.Count == 0)
        {
            _logger.LogInformation("水表葉子層聚合 {Hour:yyyy-MM-dd HH}: 無葉子節點", dtHourStart);
            return;
        }
        ...
        _logger.LogInformation(
            "已聚合 {LeafCount} 個水表葉子 SID 的 {Hour:yyyy-MM-dd HH} 用水（寫入 {Written} 列、sparse 跳過 {Sparse} 列）",
