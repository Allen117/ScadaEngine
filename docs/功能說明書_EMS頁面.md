...
| GET | `/EMS/api/main-meter-info` | `{ hasMainMeter, mode, name, voltage, current, power, powerFactor }` — `mode='realtime-by-sid'`（實體主表：`voltage={sid,pointName,unit}`）或 `mode='aggregated'`（虛擬主表：`voltage={unit}`） |
